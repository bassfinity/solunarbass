...
def calculate_major_minor_times(solunar_data):
    ...
    if solunar_data.get('moonrise') and solunar_data.get('moonset'):
        ...
        # Major periods when moon is overhead (moonrise) and underfoot (moonset)
        major_times.append({
        ...
        transit_time2 = moonrise + (moonset - moonrise) / 2 if moonset > moonrise else moonset + timedelta(hours=6)
        minor_times.append({
        ...
        if sunrise and sunset:
            ...
            # Normalize moon phase to 0-1 range
            phase_angle = moon_phase * 2 * math.pi
            ...
            if len(major_times) >= 1:
                ...
                if minor2.date() == sunrise.date() and minor2 < sunset + timedelta(hours=2):
                    minor_times.append({
